...
def get_security_group_id(group_name, region_name, vpc_id):
...
def create_cluster(region_name, cluster_name='Airflow-' + str(datetime.now()), 
                   release_label='emr-7.0.0', master_instance_type='m5.xlarge', 
                   num_core_nodes=2, core_node_instance_type='m5.xlarge', all_nodes_subnet_id='subnet-0b9701cad5e35046e'):
    vpc_id = 'vpc-0effe370b200ba896' 
    emr_master_security_group_id = get_security_group_id('AirflowEMRMasterSG0129', region_name, vpc_id)
    emr_slave_security_group_id = get_security_group_id('AirflowEMRSlaveSG0129', region_name, vpc_id)

    # Subnet ID (same for all nodes in this example)
    subnet_id = all_nodes_subnet_id  # Replace with your subnet ID
    ...
    cluster_response = emr.run_job_flow(
        ...
        Instances={
            ...
            'Ec2SubnetId': subnet_id  # Assign the same subnet for all nodes
        },
